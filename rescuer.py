...
class Rescuer(AbstAgent):
    ...
    def go_save_victims(self, map, victims, direcao):
    ...
    def direcoes_para_percorrer(pontos):
        """Converte uma lista de coordenadas em direções a serem seguidas para percorrer os pontos."""
        direcoes = []
        direcoes_map = {
            (0, 1): "Cima",
            (0, -1): "Baixo",
        ...
        for i in range(len(pontos) - 1):
            ...
            direcao = direcoes_map.get((dx, dy))
            if direcao:
                direcoes.append(direcao)
            else:
                direcoes.append(f"Movimento desconhecido: ({dx}, {dy})")
